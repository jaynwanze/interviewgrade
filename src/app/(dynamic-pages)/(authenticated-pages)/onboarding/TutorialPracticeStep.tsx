...
export function TutorialPracticeStep() {
  ...
  const handleStartTutorial = async () => {
    ...
    try {
      ...
      router.push(`/candidate/interviews/session/${newInterview.id}?tutorial=1`);
    } catch (err) {
  ...
  const tutorialSteps = [
    {
      ...
      description: 'Our AI-driven platform helps you practice real-world job interviews with AI-driven feedback.',
    },
    {
      title: 'How It Works',
      description: 'You will be asked interview questions just like in a real interview. The AI will analyze your response.',
    },
    {
      title: 'Receive Instant Feedback',
      description: 'After answering, AI will provide real-time feedback, scoring your response based on different criteria.',
    },
    {
      title: 'Let’s Get Started!',
      description: 'Ready to begin? Click "Start Tutorial" to try a quick practice session!',
    },
  ];
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-w-md p-6">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">{tutorialSteps[step].title}</DialogTitle>
        </DialogHeader>
        <p className="text-sm text-gray-600">{tutorialSteps[step].description}</p>
